import Image from "../../assets/cuisine.png";

const Proposal = () => {
  return (
    <>
      <div className="max-w-7xl mx-auto py-15 px-4 pb-10 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-extrabold text-black-900 text-center tracking-tighter pt-14">
          Tous les services
          {/* <hr className="mt-5 mx-auto w-1/4" /> */}
        </h1>
      </div>

      {/* Title 1 */}
      <div className="max-w-7xl mx-auto py-15 px-4 pb-10 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-extrabold text-black-900 text-left tracking-tighter">
          Plats
          <hr className="mt-5 shadow-sm" />
        </h1>
      </div>

      <div className="flex flex-wrap justify-center items-center space-x-0 sm:space-x-6">
        {/* Step 1 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
          <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 2 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 3 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>
      </div>

      <div className="max-w-7xl mx-auto py-15 px-4 pb-10 pt-10 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-extrabold text-black-900 text-right tracking-tighter">
          Évèvements
          <hr className="mt-5 shadow-sm" />
        </h1>
      </div>

      <div className="mt-8 flex flex-wrap justify-center items-center space-x-0 sm:space-x-6">
        {/* Step 1 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 2 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 3 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>
      </div>

      <div className="max-w-7xl mx-auto py-15 px-4 pb-10 pt-10 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-extrabold text-black-900 text-left tracking-tighter">
          Desserts
          <hr className="mt-5 shadow-sm" />
        </h1>
      </div>

      <div className="mt-8 mb-20 flex flex-wrap justify-center items-center space-x-0 sm:space-x-6">
        {/* Step 1 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 2 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>

        {/* Step 3 */}
        <div className="max-w-sm rounded overflow-hidden shadow-lg pt-5 pb-5">
        <img className="w-full" src={Image} alt="Sunset in the mountains" />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">The Coldest Sunset</div>
            <p className="text-gray-700 text-base">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              Voluptatibus quia, nulla! Maiores et perferendis eaque,
              exercitationem praesentium nihil.
            </p>
          </div>
        </div>
      </div>
    </>
  );
};

export default Proposal;
